...
class ControladorEquipe():
    ...
    def cadastrar_equipe(self):
        ...
        if existe_equipe is False:
            ...
            for cpf in cpf_split:
                alunos_equipe.append(ControladorAluno.busca_aluno_por_cpf(cpf))
            nova_equipe = Equipe(dados_equipe["nome"], dados_equipe["curso"], alunos_equipe,  dados_equipe["pontos"],)
    ...
    def listar_equipes(self):
        for equipe in self.__equipes:
            self.__tela_equipe.mostra_equipe({"nome": equipe.nome, "curso": equipe.curso, "lista_alunos": equipe.lista_alunos, "pontos": equipe.pontos})

    def finalizar(self):
        self.__controlador_sistema.abre_tela()

    def abre_tela(self):
        lista_opcoes = {0: self.finalizar, 1: self.cadastrar_equipe, 2: self.editar_equipe, 3: self.excluir_equipe, 4: self.listar_equipes}
        while True:
